...
Seeds the shared programme fixtures from `docs/test/TST-W0-foundation.md` §1 that
the canonical recipe base needs (company Rheinwerk Chemie GmbH, UoMs + conversions,
items RW-CHM-0001…0003, work centres LINE-1/MIX-01 and LINE-1/FILL-01, operations
MIX and FILL, routing RT-COMPOUND-01, BOM-RW-CHM-0003-001). Re-running is safe;
sibling wave slices extend this module rather than replacing it.
...
`tests/acceptance/` holds per-wave target-state journeys mapped 1:1 to the TC IDs
in `docs/test/TST-W{n}-*.md`. Site-backed tests skip (never fail) when no site is
reachable, so `pytest tests` is safe on a bare checkout; the `Server tests`
workflow runs them for real.
