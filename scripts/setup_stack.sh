#!/usr/bin/env bash
# Reproducible local stack for the consolidated MES scaffold:
...
#
#   ./scripts/setup_stack.sh            # create/refresh the site with the app installed
...
log "ERPNext substrate (anchor DocTypes — installed unmodified, never forked)"
...
if ! grep -qx "rheinwerk_mes" sites/apps.txt 2>/dev/null; then
	# guard against an apps.txt written without a trailing newline
