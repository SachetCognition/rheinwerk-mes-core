...
Pinned technology stack for the consolidated Rheinwerk MES at global scale (40+ plants, regional-hub topology). Companion to `docs/30-architecture/design/HLD.md` §deployment; every row carries its rationale and the decision/evidence it traces to. Versions are the recommended pins at time of writing (2026-07) — re-validate at W0 platform bring-up and record deviations in an ADR.
...
| `rheinwerk_mes` custom app | Versioned with the repo; semantic versioning from W0 | Integrity + chemicals layers live here (docs/30-architecture/ARCHITECTURE.md); anchor DocTypes are never forked, so upgrades of the anchor stay cheap. |
