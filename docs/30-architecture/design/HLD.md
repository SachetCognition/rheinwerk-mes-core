...
**Scope:** the TO-BE system only — the consolidated MES built as the `rheinwerk_mes` Frappe app on the ERPNext base (ADR-001), with the group ERP across an interface boundary (ADR-002). Inputs: `docs/30-architecture/ARCHITECTURE.md`, `docs/40-planning/CONSOLIDATION.md`, ADR-001…010, the dossier (`docs/10-discovery/dossier/production-systems-dossier.md`, Parts 4–7), the target capability model (`docs/30-architecture/target-model/target-capability-model.md`, T1–T30), the canonical model (`docs/30-architecture/canonical-model/README.md`, CDM-01…08) and the wave backlogs (`docs/40-planning/`).

Companion document: `docs/30-architecture/design/LLD.md` (low-level design of the integrity + chemicals layers).
...
- **Anchor (ERPNext as shipped).** The DocTypes listed in `docs/30-architecture/ARCHITECTURE.md` §Layering-1. Anchor hard stops are kept and verified, not re-implemented (W1-3): over-production errors (`services/status.py:29-47`, `:208-224`), stopped-WO freeze (`job_card.py:904-910`), closed-terminal (`work_order.py:1131-1132`), expired-batch throw (`stock_ledger_entry.py:287-299`).
...
- **Boundary module (`integration`).** Group-ERP contract and the SCADA adapter's MES-side endpoints. No finance/buying/selling logic in this repo (ADR-002; `docs/30-architecture/ARCHITECTURE.md` §Layering-4).

---

## 3. Layer responsibilities — target capability map (T1–T30)

Home assignments are taken verbatim from `docs/30-architecture/target-model/target-capability-model.md`; the module column maps to `docs/40-planning/CONSOLIDATION.md` "Lands in module" and `rheinwerk_mes/modules.txt`.
...
| Traceability | Full forward + backward trace as system-of-record objects, incl. blocked-batch propagation — demonstrable multi-level (W2-9 exit) | `docs/30-architecture/ARCHITECTURE.md` non-negotiables; implication 3 |
...
Characterisation tests encode legacy behaviour (Qcadoo gates, validators, picking order — W0-6) as executable parity contracts; every absorbed rule in the LLD cites its characterisation reference. Intentional divergences (e.g. estate-wide expiry hard stop, W1-9) are documented per gate (W1-10). Wave-exit acceptance tests per `docs/40-planning/*.md` exit criteria. Full strategy: LLD §9.
