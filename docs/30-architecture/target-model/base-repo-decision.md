...
5. **Consequence for repo layout:** `rheinwerk-mes-core` hosts a custom Frappe app (`rheinwerk_mes`) installed alongside the ERPNext base; anchor DocTypes are never forked (docs/30-architecture/ARCHITECTURE.md layering); Qcadoo/OFBiz code is never vendored in.
