...
**Sources:** `docs/40-planning/planning-boundary/W3-planning-boundary.md` (W3-1…W3-7) · `docs/40-planning/CONSOLIDATION.md` · `docs/30-architecture/adr/ADR-002` · `docs/30-architecture/canonical-model/README.md` (CDM-02, CDM-03, CDM-08) · `docs/10-discovery/dossier/production-systems-dossier.md` §3.1/§3.2, §5.3, §6.2, §6.3, §7, §8.2 · `.agents/skills/rheinwerk-mes-design/SKILL.md`
...
- Order state machine, execution gating, recipe governance, warehouse fidelity base — delivered in W1 (`docs/40-planning/production-core/W1-production-core.md`); W3 consumes them as dependencies.
- Genealogy, batch blocking, QI, CoA, ISA-88, hazmat master data (UN numbers, SDS references, storage classes on Item/Batch) — delivered in W2 (`docs/40-planning/traceability-quality/W2-traceability-quality.md`); W3 only *completes* hazmat at the shipping/label boundary (W3-6).
- Per-plant cutover, data backfill, legacy read-only/archival, decommission evidence — W4 (`docs/40-planning/cutover-decommission/W4-cutover-decommission.md`).
...
**Lineage:** White space · none (absent in all three sources) · dossier §6.2 "Finite capacity scheduling … still no optimiser anywhere", §6.3; build-vs-buy is business decision D4 (`docs/40-planning/consolidation-project-plan.md` Dependencies D4)
**Acceptance criteria:**
...
**Priority:** Must
**Lineage:** Rebuild (boundary) · none · ADR-002 "interface fixtures tested in Wave W3"; wave exit criterion (`docs/40-planning/planning-boundary/W3-planning-boundary.md`)
**Acceptance criteria:**
...
- **Partner-master ownership at the boundary** (are supplier/customer masters owned by group ERP with MES holding references only?) — business decision D5 (`docs/40-planning/consolidation-project-plan.md`); no legacy evidence determines the answer. Deferred to the D5 sign-off; URS-W3-010's demand model assumes references-only and must be revisited if D5 decides otherwise.
...
Restated from `docs/40-planning/planning-boundary/W3-planning-boundary.md` ("**Exit:** planning journey complete; ERP interface contract tested with fixtures") and decomposed:
