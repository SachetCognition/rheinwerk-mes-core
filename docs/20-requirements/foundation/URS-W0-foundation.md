...
Wave: W0 (Foundation) · Source backlog: `docs/40-planning/foundation/W0-foundation.md` (W0-1…W0-8) · Status: Draft for review

---

## 1. Purpose & scope

Wave W0 delivers the platform foundation of the consolidated Rheinwerk MES: the scaffolded Frappe app `rheinwerk_mes` with CI, the canonical master-data entities (Item/Product with UoM conversions, Work Centre, BOM/recipe and production-order anchors), master-data migration tooling with round-trip fixtures for all three legacy sources (Qcadoo/Plant A, OFBiz/Plant B, ERPNext/Plant C), the characterisation-test harness that encodes Qcadoo parity contracts, the evidence-pack generator, and the recorded naming/numbering scheme decision. W0 exit means: canonical entities live, master data from all three sources round-trips, and the regression floor is executing (`docs/40-planning/foundation/W0-foundation.md`, exit statement).

**Out of scope for W0** (delivered by later waves — listed so scope creep is visible):

- Production-order `exec_state` workflow, execution gating hooks, recipe governance workflow, warehouse physical fidelity (Handling Unit / Storage Location / disposal algorithms), draft reservations, shop-floor operator journey, role model, expiry policy decision — **W1** (`docs/40-planning/production-core/W1-production-core.md`).
- Batch genealogy, unified batch object with `qa_state`, batch blocking/quarantine, Quality Inspection wiring, CoA, ISA-88 recipes, hazmat data — **W2** (`docs/40-planning/traceability-quality/W2-traceability-quality.md`).
- Planning/MRP, finite capacity, group-ERP interface, SCADA/OPC-UA — **W3** (`docs/40-planning/planning-boundary/W3-planning-boundary.md`).
- Per-plant cutover, open-batch/genealogy/history backfills, legacy decommission — **W4** (`docs/40-planning/cutover-decommission/W4-cutover-decommission.md`). W0 migrates **master data only**; transactional and historical data migration is W2/W4.
...
- **Priority:** Must
- **Lineage:** Adopt · ERPNext substrate · dossier ch. 3.2 §D (healthiest platform: 515 test files, CI workflows); ADR-001; module list from `docs/40-planning/CONSOLIDATION.md`
- **Acceptance criteria:**
...
- **Priority:** Must
- **Lineage:** Adopt · ERPNext master data · dossier §5.2 (master data Rich in all three); §6.2 master-data row; `docs/30-architecture/canonical-model/README.md` conventions
- **Acceptance criteria:**
...
- **Priority:** Must
- **Lineage:** Migration · source ERPNext · dossier §7 implication 4; CDM mapping legend (`=` direct) `docs/30-architecture/canonical-model/README.md`
- **Acceptance criteria:**
...
- **Priority:** Must
- **Lineage:** Migration (Retire OFBiz per docs/40-planning/CONSOLIDATION.md) · source OFBiz · dossier §7 implication 4 and implication 9; ADR-010 consequence; ch. 3.3 (`product-entitymodel.xml`, `accounting-entitymodel.xml:630`)
- **Acceptance criteria:**
...
- **Priority:** Must
- **Lineage:** Migration · all three sources · W0-5 backlog item; dossier §7 implication 4; wave exit statement `docs/40-planning/foundation/W0-foundation.md`
- **Acceptance criteria:**
...
- **Priority:** Should
- **Lineage:** Programme tooling (no disposition) · — · `docs/50-verification/evidence/README.md` audit spine; W0-7 backlog item
- **Acceptance criteria:**
...
- **Priority:** Must
- **Lineage:** Design conformance · `.agents/skills/rheinwerk-mes-design/SKILL.md` §"Content and language" (German-first i18n discipline, DD.MM.YYYY, kg) · dossier ch. 3.1 §B.1 (legacy locale files incl. de)
- **Acceptance criteria:**
...
- **Priority:** Should
- **Lineage:** Migration tooling NFR · derived from W0-5 and the CI regression-floor exit statement (`docs/40-planning/foundation/W0-foundation.md`)
- **Acceptance criteria:**
...
- **Source → canonical mapping:** per CDM sections — Item/UoM (CDM-03 conventions, `docs/30-architecture/canonical-model/README.md` §CDM-03 mapping), Work Centre (CDM-08 mapping table), BOM headers (CDM-04 mapping), warehouse structure (CDM-03). Mapping legend `=`/`≈`/`∅`/`✕` applies; every `≈` transform is documented in the extractor.
...
Restated from `docs/40-planning/foundation/W0-foundation.md` ("canonical entities live; master data from all three sources round-trips; regression floor executing") and decomposed:
