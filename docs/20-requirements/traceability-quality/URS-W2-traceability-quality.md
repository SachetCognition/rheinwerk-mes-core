...
- **Priority:** Must
- **Lineage:** Rebuild (white space — dossier §6.3, ch. G "ISA-88: Absent" in all three) · ADR-006/CDM-04 (governed BOM+Routing pair) · docs/40-planning/CONSOLIDATION.md `recipe_isa88`
- **Acceptance criteria:**
...
- **Priority:** Must
- **Lineage:** Rebuild (white space) · wave exit criterion "recipe scaling functional" (`docs/40-planning/traceability-quality/W2-traceability-quality.md`)
- **Acceptance criteria:**
...
- **Priority:** Must
- **Lineage:** Rebuild (white space — dossier §6.3; ch. G "Hazmat: Absent" ×3, searches documented) · CDM-01 `hazmat_profile` field · docs/40-planning/CONSOLIDATION.md `regulatory_hazmat` (completes in W3)
- **Acceptance criteria:**
...
- **Priority:** Must
- **Lineage:** Wave exit criterion · `docs/40-planning/traceability-quality/W2-traceability-quality.md` W2-9 · dossier §5.3 "Trace a defect batch" journey
- **Acceptance criteria:**
...
- **Source→canonical mapping reference:** CDM-01 mapping table (`docs/30-architecture/canonical-model/README.md` §CDM-01): `advancedGenealogy.Batch.number` + `Resource.batch` → `batch_id`; `BatchState` TRACKED→Released, BLOCKED→Blocked, `Resource.blockedForQualityControl`→Quarantined → `qa_state`; `Resource.expirationDate` → `expiry_date` (lot→batch: earliest wins, conflicts reported).
- **Acceptance criteria:**
...
Restated from `docs/40-planning/traceability-quality/W2-traceability-quality.md` ("full multi-level trace demonstrable; CoA generated from inspection results; recipe scaling functional") and plan M2:
