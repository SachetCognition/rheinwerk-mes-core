...
High-level plan of record for the Rheinwerk Chemie MES consolidation programme. Every row traces to a wave backlog item in `docs/40-planning/` or a RAID source in the dossier (`docs/10-discovery/dossier/production-systems-dossier.md`) and target model (`docs/30-architecture/target-model/`). **No calendar dates** — sequencing is relative (sprint numbers only). Identical content is maintained in `consolidation-project-plan.xlsx`.
...
Total indicative duration: **20 sprints** (~40 weeks) with one blended team; waves are sequential per `docs/40-planning/README.md`.
...
Open questions §8.2 #1–#7 (plant settings, genealogy completeness, Plant B lot coverage, Qcadoo toggles, regulatory reports, external syncs, Plant B WIP) must be answered before the wave that depends on them exits (cross-wave rule 3, `docs/40-planning/README.md`). They are referenced per row in the plan detail above.
