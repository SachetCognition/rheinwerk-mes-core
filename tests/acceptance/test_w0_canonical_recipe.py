...
def test_tc_w0_007_recipe_anchors_are_not_forked(site):
	"""TC-W0-007 (URS-W0-006): `BOM` and `Routing` remain substrate-owned; the app only
	adds Custom Fields to them."""
	...
	custom = site.get_all("Custom Field", filters={"dt": "BOM"}, fields=["fieldname", "module"])
	assert {row.module for row in custom if row.fieldname.startswith(("legacy", "rw_"))} == {
		"Manufacturing Core"
	}
