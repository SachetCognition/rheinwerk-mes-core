...
# `pytest tests/…` puts only `tests/` on `sys.path` (the suites are packages), and the
# bare `pytest` entrypoint — unlike `python -m pytest` — does not add the working
# directory. Without this the suites cannot import the `rheinwerk_mes` app under test.
