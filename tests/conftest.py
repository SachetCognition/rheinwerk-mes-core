...
* **site-backed** — acceptance tests that need a Frappe site with the ERPNext substrate
  and `rheinwerk_mes` installed. They request the `site` fixture, which connects to
  `$FRAPPE_SITE` (default `dev.localhost`) in `$BENCH_PATH` (default `~/frappe-bench`)
  and rolls the transaction back afterwards.
"""
