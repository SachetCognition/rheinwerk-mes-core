...
Four gates are registered through the `rheinwerk_exec_state_gates` hook documented in
...
| `material_availability_gate` | URS-W1-008 | * → In Progress | `OrderStatesListenerServicePFTD.java:580` (+ aspect `:87-99`) |
...
def material_availability_gate(context: Any) -> None:
	"""Starting an order requires every component to be available (hard gate).

	URS-W1-008 · TC-W1-009. Baseline `OrderStatesListenerServicePFTD.checkMaterialAvailability`
	(:580-620), wired by `OrderStatesListenerAspectPFTD:87-99` for the transitions into
	*accepted*/*in progress* depending on the estate's `momentOfValidation` parameter. The
	Rheinwerk estate validates at **order start** (URS-W1-008), so the gate hangs off
	* → In Progress. Availability is on-hand minus live reservations
	(`warehouse.availability.available_qty`, URS-W1-023), so stock promised to another
	order or to a draft document does not count.
	"""
...
def component_shortfalls(doc: Any) -> list[dict[str, Any]]:
	"""Per-component shortfall list for a Work Order (URS-W1-008 AC-1/AC-3).

	Required quantity is the outstanding requirement (`required_qty` minus what was already
	transferred to WIP), compared with `available_qty` — on-hand minus live reservations —
	in the component's source warehouse. Reservations the order holds itself are added back
	so an order does not block on its own reservation; reservations of *other* vouchers stay
	excluded, which is exactly URS-W1-008 AC-3.
	"""
	...
	for row in doc.get("required_items") or []:
		...
		if required <= 0:
			continue
		available = available_qty(row.item_code, warehouse) + _reserved_by_order(
...
def component_warehouse(row: Any, doc: Any) -> str | None:
	"""Warehouse a component is drawn from: its own source when that holds stock, else the
	order's WIP/source warehouse.

	Mirrors the precedence the reservation side applies (`warehouse.reservations`), whose
	own helper is module-private — see the PR note asking for it to be published.
	"""
	...
	for warehouse in candidates:
		if warehouse and ledger_balance(row.item_code, warehouse) > 0:
			return warehouse
	return row.get("source_warehouse") or doc.get("wip_warehouse") or doc.get("source_warehouse")


def _reserved_by_order(item: str, warehouse: str, work_order: str) -> Decimal:
	"""Outstanding quantity this very order reserved for itself (excluded from shortfalls)."""
	rows = frappe.get_all(
	...
	total = Decimal("0")
	for row in rows:
		total += Decimal(str(row.reserved_qty or 0)) - Decimal(str(row.delivered_qty or 0))
	return total
