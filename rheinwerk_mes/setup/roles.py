...
W0 is *per-DocType* RBAC only — workflow-state-level permissions arrive with the
W1 execution gating. Three programme roles are created from code and layered onto
the anchors through Custom DocPerm rows via `frappe.permissions.add_permission`,
which first copies the substrate's standard permissions into Custom DocPerm so no
upstream ERPNext role loses access.

* technologist — creates/maintains canonical master data
* planner — read-only on master data, owns production orders
...
# (role, doctypes, permissions) — later rules never widen an earlier role's access.
PERMISSION_MATRIX = (
